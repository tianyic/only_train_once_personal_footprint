...
Our next-version OTO library will be released within **before Chrismas**. The new library has brand new designs along with a lot of major improvements. I recommend to **await for the coming release for new experiments**. The current repo will be moved into [legacy_branch](https://github.com/tianyic/only_train_once/tree/otov2_legacy_backup) for academic replication later on.  
