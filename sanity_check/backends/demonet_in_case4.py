...
class DemoNetInstanceNorm2DCase4(nn.Module):
    def __init__(self):
        super(DemoNetInstanceNorm2DCase4, self).__init__()
        ...
        self.prelu_2 = nn.PReLU(num_parameters=128)
        self.prelu_3 = nn.PReLU(num_parameters=256)
        self.prelu_4 = nn.PReLU(num_parameters=512)
    ...
    def forward(self, x):
        x = self.prelu_1(self.in_1(self.conv1(x)))
        x = self.prelu_2(self.in_2(self.conv2(x)))
        x = self.prelu_3(self.in_3(self.conv3(x)))
        x = self.prelu_4(self.in_4(self.conv4(x)))
        x = self.avg_pool(x)
        x = x.view(x.size(0), -1)
